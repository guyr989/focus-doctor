...
export class VerdictCache {
  ...
  get(key: string, nowMs: number): Verdict | null {
    const row = this.store.getVerdict(key);
    if (!row || row.createdAt + this.ttlSeconds < Math.floor(nowMs / 1000)) return null;
    return row.verdict;
  }

  set(key: string, taskId: number, verdict: Verdict, reason: string, nowMs: number): void {
    this.store.putVerdict({key, taskId, verdict, reason, createdAt: Math.floor(nowMs / 1000)});
  }
}
